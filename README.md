...
- **设置 → 软件更新**：显示当前版本号，「检查更新」一键查询 GitHub 最新版，
  发现新版后按钮变「立即下载」→ 下载进度 → 「重启并更新」，全程可视化
- **便签头部绿色按钮**：仅在有新版本/下载中/可更新时出现（平时零打扰），
  点击直达对应动作
- 托盘右键菜单同步显示更新状态，三个入口状态实时一致
...
| 点「每天」开关（v1.2.0+） | 所选时间变成**每日提醒**：每天到点自动提醒，一次设置永久有效 |
| 点任务前的圆圈 | 标记完成（划线沉底）|
| 点「删除」 | 删除该任务 |
...
录入时点亮「每天」开关再选时间，任务就变成绿色胶囊的每日任务：

- **天天自动提醒**：每天同一时刻弹出通知，提醒后自动滚到明天，无需重复设置
- **打卡式完成**：勾选只对今天生效（"今天已完成"），次日到点自动复活重新提醒
- **关机也不漏**：错过提醒后开机第一时间补弹一次（只弹当天的，不会连环轰炸）
- 删除即彻底停止；每日任务永不显示"已过期"
